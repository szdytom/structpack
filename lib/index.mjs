...
export { BASIC_TYPES, BaseTypeHandler, DeserializedResult, CompoundTypeHandler } from './type-handler.mjs';
...
export function deserializeFromBinary(viewOrBuffer, type) {
	if (!(viewOrBuffer instanceof DataView)) { // Check if not a DataView instance already
		if (ArrayBuffer.isView(viewOrBuffer)) {
			viewOrBuffer = viewOrBuffer.buffer;
		}
