...
/**
 * Handles storage and serialization of Sets.
 * @extends {BaseTypeHandler}
 * @class
 */
export class SetHandler extends BaseTypeHandler {
	constructor(value_handler) {
		super();
		this.value_handler = getHandlerObject(value_handler);
	}

	/**
	 * Gets the size of the serialized Set in bytes.
	 * @param {Set} value - The Set to calculate the size for.
	 * @returns {number} - The size of the serialized Set in bytes.
	 */
	sizeof(value) {
		let res = 4; // place for the size of the set
	...
	/**
	 * Serializes the Set and writes it to the DataView at the specified offset.
	 * @param {DataView} view - The DataView to write to.
	 * @param {number} offset - The offset to start writing at.
	 * @param {Set} value - The Set to be serialized.
	 * @returns {number} - The new offset after serialization.
	 */
	serialize(view, offset, value) {
		view.setUint32(offset, value.size, true);
	...
	/**
	 * Deserializes the Set from the DataView at the specified offset.
	 * @param {DataView} view - The DataView to read from.
	 * @param {number} offset - The offset to start reading from.
	 * @returns {DeserializedResult} - The deserialized result containing the Set and a new offset.
	 */
	deserialize(view, offset) {
		const size = view.getUint32(offset, true);
		offset += 4;
